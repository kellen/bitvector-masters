...
\bibliography{report}
\renewcommand*{\bibfont}{\small}    % reduce font size for bibliography
...
\title{Implementation of bit-vector variables in\\ a \sc{CP} solver,
with an application to the generation of cryptographic S-boxes}
...
\label{sec:linear}
%A linear combination is the sum of some set of elements multiplied by some set of constants.
%For two bit-vectors $x$ and $y$, consider the individual bits of $x$ to be the set of elements
%and the individual bits of $y$ to be the constants,
%then replace multiplication by \sc{AND}
...
%The linear combination of two bit-vectors $x$ and $y$ is the parity of their bitwise \sc{AND}, whose result is a single bit:\[
\[
\begin{aligned}
...
Substitution boxes, or \textit{S-boxes}, are used in order to obscure the relationship between the key and the ciphertext, a property called \textit{confusion}~\cite{shannon}. 
...
For a binary constraint $c$ over two bit-vector variables $x$ and $y$ with domains $\tuple{l^x,u^x}$ and $\tuple{l^y,u^y}$, the goal of propagation is to determine new domains $\tuple{\mathit{low}^x,\mathit{up}^x}$ and $\tuple{\mathit{low}^y,\mathit{up}^y}$ and to check if the constraint has failed. 
...
The propagator for equality between two bit-vector variables $x$ and $y$ of equal length is given in Algorithm~\ref{alg:equals}. 
...
% implemented: xor
The \sc{XOR} constraint holds for three bit-vector variables $x$, $y$, and $z$ of equal length if 
$x \oplus y = z$, 
...
The \sc{AND} constraint holds for three bit-vector variables $x$, $y$, and $z$ of equal length if
$x \land y = z$,
...
\end{algorithm}
\clearpage
The constraint $\channel(x,X)$ for a bit-vector variable $x$ and an integer variable $X$ 
...
The constraint $\weight(x) = y$ holds for a bit-vector variable $x$ and an integer variable $y$ when 
...
if the free bits are fixed to zero.
This is exactly the same as the lower bound, $l^x$, 
where all the free bits are taken as $0$, while
...
Otherwise, the integer variable $y$ is exactly equal to either the minimum or maximum
weight of the bit-vector $x$, and $x$ must be exactly $l^x$ or $u^x$, respectively.
...
by linear scan ($\BigO{n}$ time on the number of bits $n$),
by a "bit twiddling hack"~\cite{bithacks} ($\BigO{1}$ time with a look-up table),
or by a \cd{gcc}-builtin (also $\BigO{1}$ time, no look-up table necessary).
...
%\textit{Modelling and Programming with Gecode}, section V: \textit{Programming Variables}
...
for \cd{NONE}, \cd{FAILED}, and \cd{ASSIGNED} (aliased as \cd{VAL}).
...
\cd{ASSIGNED} (aliased as \cd{VAL}).
We additionally define propagation conditions for when a view's 
upper, lower, or both bounds change (\cd{UPPER}, \cd{LOWER}, \cd{BND}),
summarized in Table~\ref{tab:pc}.

Variable implementations must implement an \cd{assigned} function, which returns \cd{true} when the implementation becomes assigned; in the case of bit-vectors, this is when $l = u$. 
...
incremental propagation. Advisors use \textit{deltas} to communicate
changes in variable domains.
Our bit-vector variable implementation 
...
%\textit{Modelling and programming with Gecode} 
\cite{MPG:V}
...
A propagator for the S-2 non-linearity constraint is given in Algorithm~\ref{alg:nonlinear}. 
...
these implementations are identical except for their \C++ variable types.
...
The advantage of using a propagator in this case is a much reduced 
...
\section{Criterion S-3}
\label{sec:bitmodelsthree}
Criterion S-3 requires that each possible value from $0$ to $(2^m-1)$ occur at least once; this
...
We also implement a propagator for criterion S-7, as given in Algorithm~\ref{alg:sseven},
...
in the Section~\ref{sec:bitvecssevendecomp}. Instead of starting with an input difference $i$, it
...
is then incremented.
If at any point the count exceeds $\mathit{max\_count}$, the propagator
reports failure.

Like the global $\nonlinear$ constraint presented in Section~\ref{sec:globalstwo}, the 
$\xordist$ propagator reduces the number of variables to which changes must be propagated to
achieve the same constraint, and like $\nonlinear$, $\xordist$ also incrementally 
updates $T$ in order to improve execution time.
\begin{algorithm}
    \caption{Global propagator for criterion S-7}
    \label{alg:sseven}
    \begin{algorithmic}
...
\begin{itemize}
    ...
    \item A bit-vector model using the integer S-2 propagator and the decomposed S-7 constraint.
    \item A bit-vector model using the bit-vector S-2 propagator and the decomposed S-7 constraint.
    \item A bit-vector model using both the bit-vector S-2 propagator and the S-7 propagator.
\end{itemize}
...
We have corrected an error in~\cite{sboxes},
extended the $H_C$ heuristic,
presented two additional symmetries for substitution boxes,
defined several generic bit-vector propagators,
and defined propagators for the S-2 and S-7 criteria.
...
using propagators for the S-2 and S-7 constraints
in conjunction with the \cd{NONE} variable selection.

We have implemented only the subset of the bit-vector propagators
defined by 
Michel and Van Hentenryck~\cite{bitvectors}
necessary for our experiments,
and they define propagators for only a subset of logical operations. 
Future work could implement the remaining propagators for Gecode
...
We implemented the \sc{DES}-defined S-2 constraint;
a trivial to implement extension would be to use
Matsui's S-2$'$ instead~\cite{matsui}.
...
As defined, the propagators for requirements S-2 and S-7
